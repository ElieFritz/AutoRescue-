...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.error('Error loading data:', Crror);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const completedBreakdowns = breakdowns.filter((b: any) => b.status === 'completed');
  const pendingBreakdowns = breakdowns.filter((b: any) => b.status === 'pending');
  const totalRevenue = completedBreakdowns.reduce((acc: number, b: any) => acc + (b.total_amount || 0), 0);

  const recentActivity = breakdowns.slice(0, 5).map((b: any) => ({
    id: b.id,
    type: 'breakdown',
    message: `${b.title || 'Intervention'} - ${b.status}`,
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Vue d'ensemble de la plateforme Authelp</p>
      </div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">{pendingBreakdowns.length}</p>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">{completedBreakdowns.length}</p>
                  <p className="text-xs text-muted-foreground">Completees</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">{garages.length}</p>
                  <p className="text-xs text-muted-foreground">Garages actifs</p>
                </div>
              </div>
            </CardContent>
          </Card>
