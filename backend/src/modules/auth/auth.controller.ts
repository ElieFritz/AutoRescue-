...
export class AuthController {
  ...
  @Post('reset-password')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Reinitialiser le mot de passe avec le token' })
  ...
  async resetPassword(@Body() dto: ResetPasswordDto) {
    return this.authService.resetPassword(dto.newPassword, dto.accessToken);
  }

  @Post('change-password')
  @HttpCode(HttpStatus.OK)
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: 'Changer son mot de passe (utilisateur connecte)' })
  ...
  @Patch('users/:id/role')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: 'Changer le role d\'un utilisateur (admin)' })
  async updateUserRole(@Param('id') id: string, @Body() body: { role: string }, @CurrentUser() user: any) {
    // Check if user is admin
    if (user.role !== 'admin') {
